...
public class RecipeEndpointTest {
    ...
    public void shouldGetIngredients() throws Exception {
        ...
        given(service.getById(data.getId())).willReturn(Optional.of(recipe));
        mockMvc.perform(get("/recipes/" + data.getId() + "/ingredients"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.data", hasSize(2)))
    ...
    public void shouldPatch() throws Exception {
        ...
        var argument = ArgumentCaptor.forClass(Recipe.class);
